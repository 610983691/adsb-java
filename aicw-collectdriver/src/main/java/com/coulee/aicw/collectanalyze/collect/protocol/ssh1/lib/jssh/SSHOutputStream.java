 ...
 * The writePacket method compresses and encrypts the packet if appropriate.
 */
public class SSHOutputStream extends DataOutputStream implements IPacketConstants {
	/**
	 * Constructor
	 */
	...
	/**
	 */
	public synchronized void writePacket(Packet packet_) throws IOException {
		if (packet_ == null) {
			return;
		}
		...
		/*
		 * Compress the [ packet-type + data ] if compression is turned on.
		 */
		if (_deflater != null) {
	...
	/**
	 * Set the cipher algorithm (this method must be called after the session key
	 * has been exchanged and the cipher type has been negotiated).
	 */
	...
	/**
	 * Set the compression level (by default compression is off). This method is
	 * called after both side have agreed on the compression level.
	 */
	...
	/**
	 * Encode a String as a byte-array for insertion into an SSH packet. First 4
	 * bytes are the length of the string, msb first (not including the length
	 * itself). The following "length" bytes are the string value. There are no
	...
	/**
	 * Encode the specified integer into a byte array as four consecutive bytes, msb
	 * first.
	 * 
	 * @param value_
	 *            the integer to be encoded.
	 * @param offset_
	 *            the offset where the first byte must be stored. This offset is NOT
	 *            updated by the method (Java can't modify method parameters).
	 * @param bytes_
	 *            the byte array into which the integer must be inserted.
	 */
