...
/**
 * This class provides miscellaneous static methods for use in the Java SSH
 * Client.
 */
public class SSHMisc {
	...
	/**
	 * Returns a new byte array which is the concatenation of the two byte arrays
	 * supplied.
	 */
	...
	/**
	 * Returns a new byte array of the same size as the byte arrays supplied. Each
	 * element of the new array is formed by XORing the corresponding elements of
	 * the supplied arrays.
	 */
	 ...
	 */
	static private long crc32_tab[] = { 0x00000000L, 0x77073096L, 0xee0e612cL, 0x990951baL, 0x076dc419L, 0x706af48fL,
	...
	/**
	 * Compute the crc Cyclic Redundancy Check, with the polynomial 0xedb88320, The
	 * polynomial is
	 * X^32+X^26+X^23+X^22+X^16+X^12+X^11+X^10+X^8+X^7+X^5+X^4+X^2+X^1+X^0 We take
	 * it "backwards" and put the highest-order term in the lowest-order bit. The
	 * X^32 term is "implied"; the LSB is the X^31 term, etc. The X^0 term (usually
	 * shown as "+1") results in the MSB being 1. so the poly is 0x04c11db7 (used
	 * for Ethernet) The buf will be the Padding, Packet type, and Data fields. The
	 * crc is computed before any encryption. R =X^n * M rem P M message P
	 * polynomial crc R : crc calculated. T(x) = x^n * M(x) + R(x) property: T rem P
	 * = 0
	 */
