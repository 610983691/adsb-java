...
class PacketQueue extends java.util.LinkedList {
	private static final long serialVersionUID = 7461834257441313261L;
	...
	/**
	 * Dequeue an Packet object off the queue; if the queue is empty, wait until
	 * another thread enqueues an event onto it.
	 * 
	 * @return the next Packet from the queue; or <b>null</b> if the thread was
	 *         interrupted.
	 */
	synchronized Packet getNextPacket() {

		/*
		 * If the queue is empty, block until another thread enqueues an packet.
		 */
		while (super.size() == 0) {
			try {
				wait();
