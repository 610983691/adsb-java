 ...
 * It decrypts and uncompresses the packet if appropriate, and returns an
 * instance of the appropriate subclass of Packet.
 */
public class SSHInputStream extends DataInputStream implements IPacketConstants {
	/**
	 * Constructor
	 */
	...
	/**
	 * Read an SSH packet from the network. The packet is automatically decrypted if
	 * the protocol is in encrypted mode. The CRC of the decrypted packet is
	 * checked.
	 * 
	 * @return an instance of the appropriate subclass of Packet.
	 * @exception SSHProtocolException
	 *                if the CRC is incorrect, or an unknown packet type is
	 *                received.
	 ...
	 */
	public Packet readPacket() throws IOException, SSHProtocolException {
		int packet_length = 0;
		...
		System.arraycopy(block, padding_length, data, 0, data.length);
		...
		default:
			throw new SSHProtocolException("received unknown packet type " + packet_type);
		}
	}
	...
	/**
	 * Return the string that is encoded at position "offset" in the specified byte
	 * array. First 4 bytes are the length of the string, msb first (not including
	 * the length itself). The following "length" bytes are the string value. There
	 * are no terminating null characters.
	 * 
	 * @param offset
	 *            the integer offset of the start of the encoded string.
	 * @param byteArray
	 *            the byte array containing the encoded string.
	...
	/**
	 * Returns the 32-bit integer encoded at position "offset" in the specified byte
	 * array.
	 */
	...
	/**
	 * Return the multiprecision-integer at the position "offset" in the specified
	 * byte array. First 2 bytes are the number of bits in the integer, msb first
	 * (for example, the value 0x00012345 would have 17 bits). The value zero has
	 * zero bits. It is permissible that the number of bits be larger than the real
	 * number of bits. The number of bits is followed by (bits + 7) / 8 bytes of
	 * binary data, msb first, giving the value of the integer.
	 */
	static public byte[] getMpInt(int offset, byte[] byteArray) {
		...
		System.arraycopy(byteArray, offset, mpInt, 0, byteLength);
		return mpInt;
	}
