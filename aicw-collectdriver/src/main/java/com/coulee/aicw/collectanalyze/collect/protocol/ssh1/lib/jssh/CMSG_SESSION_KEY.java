 ...
 * selects the cipher to use, and sends the encrypted session key to the server.
 * The anti_spoofing_cookie must be the same bytes that were sent by the server.
 * Protocol_flags is intended for negotiating compatible protocol extensions.
 */
class CMSG_SESSION_KEY extends Packet {
	/**
	 * Construct the SSH_CMSG_SESSION_KEY packet.
	 *
	 * @param trueRandom_
	 *            an object that implements the ITrueRandom interface and supplies
	 *            truly random bits (not pseudo-random bits).
	 *
	 * @param keypacket_
	 *            the SMSG_PUBLIC_KEY packet that we received from the server.
	 */
	CMSG_SESSION_KEY(ITrueRandom trueRandom_, SMSG_PUBLIC_KEY keypacket_) throws SSHSetupException {
		...
		/*
		 * create the session id (for protocol v1.5) _session_id = md5(hostkey->n ||
		 * servkey->n || cookie)
		 */
		_session_id = new byte[host_key_public_modulus.length + server_key_public_modulus.length
				+ anti_spoofing_cookie.length];

		System.arraycopy(host_key_public_modulus, 0, _session_id, 0, host_key_public_modulus.length);

		System.arraycopy(server_key_public_modulus, 0, _session_id, host_key_public_modulus.length,
				server_key_public_modulus.length);

		System.arraycopy(anti_spoofing_cookie, 0, _session_id,
				host_key_public_modulus.length + server_key_public_modulus.length, anti_spoofing_cookie.length);
		...
		if ((supported_ciphers_mask[3] & (byte) (1 << Cipher.SSH_CIPHER_BLOWFISH)) != 0) {
			...
			if ((supported_ciphers_mask[3] & (1 << Cipher.SSH_CIPHER_IDEA)) != 0) {
				...
				if ((supported_ciphers_mask[3] & (1 << Cipher.SSH_CIPHER_3DES)) != 0) {
					...
					throw new SSHSetupException("server does not support IDEA, BlowFish or 3DES, " + "cipher mask is 0x"
		...
		/*
		 * System.currentTimeMillis() returns the number of milliseconds since January
		 * 1, 1970, 00:00:00 GMT. Math.random() a pseudorandom double between 0.0 and
		 * 1.0.
		 */
		// _md5.reset();
		 ...
		 * random_bits1)); random_bits2 =
		 * md5.hash(SSHMisc.concatenate(md5.hash(_password + _login), random_bits2));
		 */
		...
		/*
		 * The protocol spec says that "the resulting string is then encrypted with the
		 * smaller key (one with the smaller modulus), and the result is then encrypted
		 * with the other key".
		 */
		byte[] encrypted_session_key;
		if (server_key_public_modulus.length < host_key_public_modulus.length) {
			byte[] server_key_encrypted_data = RSAAlgorithm.publicKeyEncrypt(session_keyXored,
					server_key_public_exponent, server_key_public_modulus, _md5);

			encrypted_session_key = RSAAlgorithm.publicKeyEncrypt(server_key_encrypted_data, host_key_public_exponent,
					host_key_public_modulus, _md5);
		} else {
			byte[] server_key_encrypted_data = RSAAlgorithm.publicKeyEncrypt(session_keyXored, host_key_public_exponent,
					host_key_public_modulus, _md5);

			encrypted_session_key = RSAAlgorithm.publicKeyEncrypt(server_key_encrypted_data, server_key_public_exponent,
					server_key_public_modulus, _md5);
		}
		...
		// set the data
		int block_length = 1 + // packet type
				1 + // cipher_type
				anti_spoofing_cookie.length + 2 + encrypted_session_key.length + protocol_flags.length;
