...
/**
 * Sent by either party in interactive mode, this message indicates that a
 * connection has been opened to a forwarded TCP/IP port.
 */
class MSG_PORT_OPEN extends Packet implements IInteractivePacket {
	...
	/**
	 * @param local_channel_
	 *            the channel number that the sending party has allocated for the
	 *            connection.
	 */
	...
	public void processPacket(IProtocolHandler handler_) {
		...
		if (allowed) {
			...
			/*
			 * We don't want the application to exit until this thread has exited.
			 */
