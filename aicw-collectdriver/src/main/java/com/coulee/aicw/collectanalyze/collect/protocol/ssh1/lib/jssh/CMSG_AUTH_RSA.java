...
/**
 * The client sends this message containing the client's public key, in order to
 * initiate RSA authentication. If the server is willing to accept
 * authentication with the offered public key, it sends an
 * SSH_SMSG_AUTH_RSA_CHALLENGE containing random data encrypted with the
 * client's public key.
 */
class CMSG_AUTH_RSA extends Packet {
	/**
	 * Constructor.
	 */
	CMSG_AUTH_RSA(byte[] public_modulus_) {
		...
		int packet_length = 1 + 2 + public_modulus_.length;
