...
abstract class Packet implements IPacketConstants {
	/**
	 * Use the default constructor when you don't know the size of the data
	 * array in advance.
	 */
	protected Packet() {
	}
	...
	protected Packet(int type_, byte[] data_) {
		...
		_data[0] = (byte) type_;
		System.arraycopy(data_, 0, _data, 1, data_.length);
	}
	...
	/**
	 * Returns the byte array including the packet-type byte and the binary data
	 * bytes
	 */
	byte[] getData() {
		return _data;
	...
	/**
	 * This byte array represents the data block, EXCLUDING the padding bytes
	 * and the CRC (in other words, it contains just the packet-type byte and
