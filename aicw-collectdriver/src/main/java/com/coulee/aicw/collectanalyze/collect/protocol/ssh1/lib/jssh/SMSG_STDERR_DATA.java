...
/**
 * Delivers data from the server that was read from the standard error stream of
 * the shell or program running on the server side. This message can only be
 * used in the interactive session mode. No acknowledgement is sent for this
 * message.
 */
class SMSG_STDERR_DATA extends Packet implements IInteractivePacket {
	/**
	...
	/**
	 * Use this constructor when constructing a packet that must be sent to STDERR
	...
	public void processPacket(IProtocolHandler handler_) {
		/*
		 * The STDOUT_InputStream class will decide where to send the output.
		 */
