...
class OpenChannel implements Runnable, IPacketConstants {
	 ...
	 * port-forwarding. We have to send a SSH_MSG_PORT_OPEN to the remote side,
	 * containing the specified hostname and port, and wait for a
	 * SSH_MSG_CHANNEL_OPEN_CONFIRMATION in reply.
	 */
	...
	public void run() {
		if (_socket != null) {
			/*
			 * This is a local-to-remote channel, and we have received an incoming TCP
			 * connection.
			 */
			...
			/*
			 * This is a remote-to-local channel, and we have received a SSH_MSG_PORT_OPEN.
			 */
			try {
			...
			MSG_CHANNEL_OPEN_CONFIRMATION reply = new MSG_CHANNEL_OPEN_CONFIRMATION(_remoteChannelNumber,
					getLocalChannelNumber());
			_handler.enqueueToRemote(reply);
		}
		...
		try {
			...
			/*
			 * We don't want the application to exit until this thread terminates.
			 */
			...
			for (;;) {
				...
				if (type == SSH_MSG_CHANNEL_DATA) {
					...
					MSG_CHANNEL_CLOSE_CONFIRMATION confirmation = new MSG_CHANNEL_CLOSE_CONFIRMATION(
	...
	/**
	 * A nonstatic inner class that runs in its own thread, reading data from the
	 * network and forwarding it to the remote side via the encrypted channel.
	 */
