...
/**
 * This class receives SSH_SMSG_STDOUT_DATA packets on its input queue, and
 * makes the received data available via an InputStream interface.
 */
public class STDOUT_InputStream extends InputStream implements IPacketConstants {
	/**
	 * Constructor
	 */
	...
	/**
	 * Reads some number of bytes from this input stream and stores them in the
	 * array b_.
	 * 
	 * @return the number of bytes read into the buffer, or -1 if there was no more
	 *         data because the end of the stream was detected.
	 */
	@Override
	...
	/**
	 * Reads up to <code>len_</code> bytes of data from this input stream into the
	...
	/**
	 * This private method is called when the output buffer is empty. It blocks
	 * until there is data available to refill the output buffer.
	 */
	private void _refillOutputBuffer() throws IOException {
		...
		if (packet == null) {
			/*
			 * The thread that was reading from the packet-queue was interrupted.
			 */
		...
		if (type == SSH_SMSG_STDOUT_DATA) {
			...
			throw new RuntimeException("STDOUT_InputStream received unexpected packet type " + type);
		}
	}
