...
/**
 * This message is sent by the client to request that a pseudo-terminal device
 * be allocated for this session. The user terminal type and terminal modes are
 * supplied as arguments.
 */
class CMSG_REQUEST_PTY extends Packet {
	 ...
	 * @param modes_
	 *            the terminal modes, encoded as specified in the SSH protocol 1.5
	 *            spec (page 30 onwards).
	 */
	CMSG_REQUEST_PTY(String terminal_, int rows_, int columns_, byte[] modes_) {
		super();

		byte[] terminal_type = SSHOutputStream.string2Bytes(terminal_);

		int block_length = 1 + // packet-type byte
