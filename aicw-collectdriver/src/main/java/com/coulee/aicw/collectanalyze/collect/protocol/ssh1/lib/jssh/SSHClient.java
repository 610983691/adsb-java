...
/**
 * This is an example of a program that uses the JSSH library to connect to a
 * remote host using the SSH protocol (version 1.5).
 * <p>
 * 
 * This program should be run from the script "test.sh", which disables echoing
 * on the input stream, and also disables processing of control characters on
 * input.
 * 
 * This particular example uses the stdin and stdout streams, and starts up two
 * threads (implemented by the nonstatic inner classes ReaderThread and
 * WriterThread) to read and write these streams. Instead of using the stdin and
 * stdout streams, one could start up a terminal-emulator (assuming that the
 * emulator provides an input stream and an output stream) and read and write
 * the terminal-emulator's streams.
 */
public class SSHClient {
	...
	public void connect() throws Exception {
		...
		/*
		 * Start up a separate thread to terminate the authentication process if it
		 * doesn't complete within 60 seconds.
		 */
		TimeoutThread timeout_thread = new TimeoutThread();
		timeout_thread.setDaemon(true);
		timeout_thread.start();

		socket.connect(new InetSocketAddress(serverAddress, _options.getPort()), _options.getConnectTimeout());
		...
		/*
		 * Start a thread to read debugging information from the STDOUT_InputStream
		 */
		...
		/*
		 * Now we are connected to the server, exchange version information as required
		 * by the SSH protocol.
		 */
		...
		/*
		 * Get the username - either from the command-line, or interactively if the
		...
		/*
		 * The setup exchange is complete; we can now start the emulator.
		 */
	...
	private static void getOptions(String[] argv_, Options options_) {
		for (int i = 0; i < argv_.length; i++) {
			if (argv_[i].startsWith("-") == false) {
				...
				/*
				 * Parse the answerback string and look for escape chars such as "\r" and "\t".
				 * The standard C/Java escape characters are handled.
				 */
	...
	/**
	 * A nonstatic inner class that is used to time-out the authentication process
	 * after 60 seconds.
	 */
	...
	/**
	 * A nonstatic inner class that reads from the STDOUT_InputStream and copies to
	 * the stdout stream.
	 */
	...
	/**
	 * A nonstatic inner class that reads from the keyboard and copies to the
	 * STDIN_OutputStream
	 */
	@SuppressWarnings("unused")
	...
	public STDIN_OutputStream getSTDIN() {
		return this._handler.getSTDIN();
	}

	public STDOUT_InputStream getSTDOUT() {
		return this._handler.getSTDOUT();
	}

	private void startShell() throws IOException, SSHProtocolException, InterruptedException {
	...
	public void close() throws IOException {
		this._handler.stopShell = true;
		while (this._handler.isAllStoped()) {
			this._handler.getStdout().close();
			this._handler.getStdin().close();
			this.socket.close();
			break;
		}
	}
