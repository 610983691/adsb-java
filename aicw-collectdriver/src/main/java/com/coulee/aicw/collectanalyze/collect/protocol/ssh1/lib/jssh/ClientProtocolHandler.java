...
import com.coulee.aicw.collectanalyze.collect.protocol.ssh1.lib.cipher.Cipher;
 ...
 */
public class ClientProtocolHandler implements Runnable, IProtocolHandler,
		IPacketConstants {
	...
	/**
	 * Returns a reference to the InputStream object from which data must be
	 * read (for displaying on the screen).
	 */
	...
	/**
	 * Returns a reference to the OutputStream object to which keyboard data
	 * must be written (for sending to the server).
	 */
	 ...
	 * 
	 * @exception SSHSetupException
	 *                if the server's version string is incompatible.
	 */
	...
	/**
	 * Wait for the SSH_SMSG_PUBLIC_KEY packet from the server. This method must
	 * be called after <code>exchangeIdStrings()</code> has returned
	 * successfully.
	 */
	public void receiveServerKey() throws IOException, SSHSetupException,
	...
	/**
	 * Create and send an encrypted session key; this method must be called
	 * AFTER receiveServerKey() has returned successfully.
	 * 
	 * @param trueRandom_
	 *            an object that implements the ITrueRandom interface and
	 *            supplies truly random bits for the session key. Note that it
	 *            is <strong>not</strong> sufficient to use a pseudo-random
	 *            number generator (PRNG) seeded by the system clock; for an
	 *            explanation, see <a
	 *            href="http://www.ietf.org/rfc/rfc1750.txt">rfc1750 (Randomness
	 ...
	 *            ITrueRandom interface. On other operating systems you will
	 *            have to provide your own source of true random bits. See <a
	 *            href="http://www.cs.berkeley.edu/~daw/rnd/">Randomness for
	 *            Crypto</a> for information on generating sufficiently random
	 *            numbers.
	 */
	public void sendSessionKey(ITrueRandom trueRandom_)
		 ...
		 */
		CMSG_SESSION_KEY session_key_packet = new CMSG_SESSION_KEY(trueRandom_,
				_server_public_key_packet);
	 ...
	 */
	public boolean declareUser(String username_) throws IOException,
			SSHProtocolException {
	 ...
	 * must be called after declareUser() has returned successfully with a
	 ...
	 * @exception SSHProtocolException
	 *                if a malformed/invalid SSH packet was received.
	 */
	public boolean authenticateUser(String user_, String password_)
		...
		/*
		 * The server sent us SSH_SMSG_FAILURE, which means that authentication
		 * is required.
		 */
	...
	/**
	 * Authenticate the user using RSA authentication. This method must be
	 * called after declareUser() has returned successfully with a return value
	 * of <code>true</code>.
	 * 
	 ...
	 * @exception SSHProtocolException
	 *                is a malformed/invalid SSH packet is received.
	 */
	public boolean authenticateUser(RSAPrivateKey privateKey_)
		...
		/*
		 * Now we must decrypt the challenge using our private key, compute the
		 * MD5 checksum of the challenge plus the session id, and send back the
		 * resulting 16 bytes.
		 */
		byte[] serverRandomBytes = RSAAlgorithm.encrypt(challenge,
				privateKey_.getExponent(), privateKey_.getModulus());
		...
		CMSG_AUTH_RSA_RESPONSE response_packet = new CMSG_AUTH_RSA_RESPONSE(
				response_bytes);
	...
	public void preparatoryOperations() throws IOException,
			SSHProtocolException, SSHSetupException {
		if (_phase != PHASE_PREPARATORY) {
			throw new RuntimeException("Protocol not in preparatory phase");
		}

		byte[] terminalModes = { CMSG_REQUEST_PTY.ICRNL, 1,
				CMSG_REQUEST_PTY.ISIG, 1, CMSG_REQUEST_PTY.ICANON, 1,
				CMSG_REQUEST_PTY.ECHO, 1, CMSG_REQUEST_PTY.ECHOE, 1,
				CMSG_REQUEST_PTY.ECHOK, 1, CMSG_REQUEST_PTY.ECHONL, 1,
				CMSG_REQUEST_PTY.OPOST, 1, CMSG_REQUEST_PTY.ONLCR, 1,
				CMSG_REQUEST_PTY.CS8, 1, CMSG_REQUEST_PTY.TTY_OP_END };

		int columns = _options.getTerminalSize().width;
		int rows = _options.getTerminalSize().height;
		CMSG_REQUEST_PTY pty_packet = new CMSG_REQUEST_PTY(
				_options.getTerminalType(), rows, columns, terminalModes);
		...
		while (iter.hasNext()) {
			...
			try {
				...
				throw new java.net.BindException("Cannot bind to port "
						+ pf.getListenPort() + "; " + e.getMessage());
			}
		...
		while (iter.hasNext()) {
			PortForwarding pf = (PortForwarding) iter.next();
			debug("Requesting forwarding of remote port " + pf.getListenPort()
					+ " to " + pf.getHostname() + ":" + pf.getHostPort());

			CMSG_PORT_FORWARD_REQUEST forward_request = new CMSG_PORT_FORWARD_REQUEST(
			...
			if (readNextPacket().getPacketType() != SSH_SMSG_SUCCESS) {
				throw new SSHSetupException("Server refused to forward port "
						+ "(listen-port=" + pf.getListenPort() + " hostname="
						+ pf.getHostname() + " host-port=" + pf.getHostPort()
						+ ")");
			}
		}
		...
		if (_options.compressionEnabled()) {
			...
			CMSG_REQUEST_COMPRESSION compression_request = new CMSG_REQUEST_COMPRESSION(
					6);
	 ...
	 */
	public synchronized void execShell() throws IOException, SSHProtocolException {
		...
		/*
		 * Start up a separate thread to read (keystroke) packets off the client
		...
		/*
		 * Read packets from the network input stream, decrypt them and process
		 * them.
		 */
		debug("Entering interactive session.");
		while (true) {
			if (stopShell) {
				System.out.println("readerStop ../././././././");
				readerStop = true;
				break;
			}
			Packet packet = null;
			...
			if (packet instanceof IInteractivePacket == false) {
				throw new SSHProtocolException("packet type "
						+ packet.getPacketType()
	 ...
	 * On UNIX, the command is run as "&lt;shell&gt; -c &lt;command&gt;" where
	 * &lt;shell&gt; is the user's login shell.
	 * <p>
	 * 
	 * This method is an alternative to <code>execShell()</code>. It must be
	 * called after the preparatory operations have completed successfully.
	 */
	public void execCmd(String command_) throws IOException,
			SSHProtocolException {
		if (_phase != PHASE_READY) {
			throw new RuntimeException("Protocol not in correct state");
		}
	...
	/**
	 * This method runs in a separate thread, and handles the sending of SSH
	 * protocol packets to the SSH server. It is started automatically when the
	...
	/**
	 * Private helper method to read the next SSH packet. DEBUG packets are
	 * logged but are not returned to the caller.
	 */
		...
		public void run() {
			for (;;) {
				...
				try {
					...
					/*
					 * The sleep was interrupted by transmission or reception of
					 * a packet. Start a new sleep.
					 */
					continue;
				}
				/*
	...
	/**
	 * The public key packet received from the server is stored here for later
	 * use.
	 */
	...
	public SSHInputStream getStdout() {
		return this._ssh_in;
	}

	public SSHOutputStream getStdin() {
		return this._ssh_out;
	}

	public void setStdout(SSHInputStream in) {
		this._ssh_in = in;
	}

	public void setStdin(SSHOutputStream out) {
		this._ssh_out = out;
	}
	 ...
	 * shell线程开关
	 */
	volatile boolean stopShell = false;
	
	/**
	 * 发送命令线程是否停止标识
	 */
	private boolean senderStop = false;

	/**
	 * 接收回显线程是否停止标识
	 */
	private boolean readerStop = false;

	/**
	 * 检测session连接线程是否停止标识
	 */
	private boolean keepAliveStop = true;
	
	public boolean isAllStoped() {
		return senderStop && readerStop && keepAliveStop;
	}
}
