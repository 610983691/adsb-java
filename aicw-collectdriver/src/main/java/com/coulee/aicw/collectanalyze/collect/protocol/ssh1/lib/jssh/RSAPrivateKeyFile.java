...
/**
 * This class encapsulates the properties and behaviour of an SSH RSA
 * private-key file. The file has the following structure:
 * <p>
 *
 * <UL>
 ...
 * <LI>A comment string.
 * <LI>The remaining bytes of the file contain the private key, encrypted (if
 * the cipher type is nonzero) with the specified cipher.
 * </UL>
 */
...
public class RSAPrivateKeyFile {
	...
	public RSAPrivateKeyFile(String filename_) throws IOException {
		...
		if ((_cipherType != Cipher.SSH_CIPHER_NONE) && (_cipherType != Cipher.SSH_CIPHER_3DES)) {
	...
	/**
	 * This method should be called if the key is not encrypted, ie getCipherType()
	 * returns SSH_CIPHER_NONE;
	 */
	...
	/**
	 * This method should be called if the key is encrypted, ie if getCipherType()
	 * returns SSH_CIPHER_3DES.
	 */
	...
	/**
	 * Private method that extracts the private key from the decrypted buffer.
	 */
	...
	/*
	 * public static void main(String[] args) { try { RSAPrivateKeyFile keyfile =
	 * new RSAPrivateKeyFile(System.getProperty("user.home") + "/.ssh/identity");
	 * RSAPrivateKey key = keyfile.getPrivateKey();
	 * System.err.println(SSHMisc.debug(key.getModulus())); } catch (Exception e) {
	 * e.printStackTrace(); } }
	 */
