...
/**
 * This class encapsulates all the options that can be specified on the
 * command-line for the SSH client.
 */
...
public class Options {
	...
	/**
	 * Adds the specified local port forwarding to the list of local ports to be
	...
	/**
	 * Adds the specified port-forwarding to the list of server ports that must be
	...
	/**
	 * Returns an iterator that allows the calling code to iterate through the list
	 * of local port-forwardings.
	 */
	 ...
	 * 
	 * @param hostname_
	 *            the hostname contained in the SSH_MSG_PORT_OPEN.
	 * @param port_
	 *            the port contained in the SSH_MSG_PORT_OPEN.
	 */
	public boolean isPortOpenAllowed(String hostname_, int port_) {
		...
		while (iter.hasNext()) {
			...
			if (pf.getHostname().equals(hostname_) && pf.getHostPort() == port_) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Set the terminal-type string that will be sent in the SSH_CMSG_REQUEST_PTY
	 * packet.
	 */
	...
	/**
	 * Sets the interval (in seconds) between keepalive packets. In some networks,
	 * firewalls or routers need to keep track of "sessions", and can lose track of
	 * a session if they don't see a packet on that session within some time period.
	 * Setting the keppalive timeout to a nonzero value causes a SSH_MSG_IGNORE
	 * packet to be sent if the TCP connection has been idle for the specifid number
	 * of seconds.
	 */
	...
	private String _identity = System.getProperty("user.home") + "/.ssh/identity";
