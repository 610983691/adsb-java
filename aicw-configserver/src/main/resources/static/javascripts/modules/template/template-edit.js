$(function () {
	...
	selData.isInner = selData.isInner + '';
	$('.form-horizontal').formEdit(parent.selData);

	var valuesObject = {};
	valuesObject['id'] = selData.id;
	$.ajax({
		type : "POST",
		url : '/template/detail',
		cache : false,
		...
		data : JSON.stringify(valuesObject),
				...
				if (itemCount == 1) {
					...
					newRow +=		'</div>'+
								...
								'<div class="row form-group">'+
									...
									'<div class="col-sm-4">'+
										'<input type="hidden" name="id' + itemCount + '" value="' + this.id + '"/>'+
								...
								'<div class="row form-group">'+
									...
									'<div class="col-sm-4">'+
										'<input type="' + (this.itemIsCrypt == "1" ? 'password' : 'text') + '" class="form-control" name="itemValue' + itemCount + '" value="' + this.itemValue + '"/>'+
									'</div>'+
								'</div>'+
							'</td></tr>';
				$(".table").append(newRow);
    ...
    $('.btn-success').click(function(){
		...
		$.each(trs, function() {
			...
			var record = {};
			record.id = $(this).find('input[name^="id"]').val();
